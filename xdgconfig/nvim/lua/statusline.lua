...
local statusline_node_type = v.statusline_node_type
...
local function ts_display_node(type, name, sep)
	if ts_colours['hiddennodes'][type] == true then
		return ''
	end

	for hg, node_types in pairs(ts_colours) do
		local found = node_types[type]
		if found and found ~= true then
			name = name or found
		end
		if found then
			return sep
        ...
        if vim.g[statusline_node_type] ~= "" then
	return sep .. '(' .. type .. ')'
        else 
                return ""
        end
...
local function get_displayed_file_name()
	local filepath = vim.g[statusline_filepath]
	if filepath == 'path' then
		return vim.fn.expand('%:p')
	elseif filepath == 'dir' then
		return vim.fn.expand('%:p:h:t') .. '/' .. vim.fn.expand('%:f')
	elseif filepath == 'file' then
...
local function sl_battery()
	...
	if status == 'Charging' then
		hg = make_hg('CustomStatusLineBatteryCharging')
	else
		hg = make_hg('CustomStatusLineBatteryDischarging')
	end
	return sl_sep() .. hg .. string.format('%s', level)
end

local function sl_position()
	if vim.g[statusline_linecol] == '' then
		return ''
	end
...
local function sl_messages()
	...
	if has_reg and has_notif then
		inner_sep = sl_sep()
	end
	if has_reg or has_notif then
		outer_sep = sl_sep()
	end
