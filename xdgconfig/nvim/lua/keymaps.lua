...
setk(
        n,
        'ùs',
        ':so ' .. os.getenv('XDG_CONFIG_HOME') .. '/nvim/lua/statusline.lua<CR>',
        'reload statusline'
)
...
for letter, command in pairs({
        ...
        local desc = winicon .. command
        setknod(nit, keymap, '<C-\\><C-n>' .. command, desc)
        keymap = '<D-' .. letter .. '>'
        setknod(n, keymap, command, desc)
        setknod(it, keymap, '<C-\\><C-n>' .. command, desc)
        add_keymap_to_doc(nit, keymap, command, desc)
end
...
for raw_letter, cmd in pairs({
        ...
        for leti, letter in ipairs({ raw_letter, string.upper(raw_letter) }) do
                setk(n, 'é' .. letter, function()
                        vim.o.laststatus = 0
                        local opts = T3(leti == 2, {}, { cwd = vim.fs.root(0, { '.git' }) })
...
for _, folder in ipairs(icons) do
        setk(
                n,
                'è' .. folder.short,
                function() require('oil').open(folder.val) end,
                oilicon .. folder.icon .. folder.val
        )
end
...
setk(n, 'ày', function() vim.lsp.buf.type_definition() end, 'Go to type definition')

--> lsp status <--

setk(n, 'às', ':checkhealth lsp<CR>')
setk(n, 'ào', ':ConformInfo<CR>')
...
for key, scope in pairs({
        ...
        local desc = tsoicon .. scope:sub(2)
