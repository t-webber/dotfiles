...
#define SHORTP(path, pwd, var, alias)                      \
        SHORT(path,                                        \
              alias,                                       \
              strcmp(getenv_checked(var), pwd) == 0);
...
static void wait_and_suspend(void) {
        ...
        if (pid < 0) {
                panic("suspend failed (0), but plug it "
                      "in.\n");
        }

        if (pid == 0) {
                printf("\033[31mBattery running out. Plug "
                       "your "
                       "computer.\033[0m\n");
                setsid();
                sleep(60);
                ...
                execl("/bin/sudo",
                      ...
                      NULL);
                panic(
                    "suspend failed (1), but plug it in\n");
        }
...
static void get_battery_string(str battery,
                               const_str device) {
...
int main(void) {
        ...
        if (!strcmp(device_name, "acer")
            || !strcmp(device_name, "mac"))
                device_name = "";
