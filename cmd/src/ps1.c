...
static void pwd(str path) {
        read_simple_exl(512, pwd, "pwd", "pwd");
...
static void low_battery(void) {
        ...
        if (pid_outer == 0) {
                ...
                if (pid_inner == 0) { exl_err_notif_msg("Low battery"); }
                ...
                if (!strcmp(status, "Discharging") && level < 10) {
                        exl("/bin/sudo", "sudo", "systemctl", "suspend", NULL);
                }
...
int main(void) {
        ...
        if (!is_charging && is_acer && atoi(battery) < 10) low_battery();
