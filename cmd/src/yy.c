...
// TODO: allow more complex libcmd arg1 (y ci? file.py)
...
static const_str USAGE = U
        ...
        NL PROG("y") S OPT("n,y,i,r,a,v,t,c,e,p,s,m,f,l") OPTS
        NL "\tveNv sYnc Init Rm Add serVe pyTest C rEpl Pip Streamlit Module ruFf Lsp(ty)"
...
int main(const int argc, Args argv) {
        ...
        const_str py = get_venv("python");

#define exm(offset, prog, ...)                                                                     \
        manual_extend_exec(argv + offset, (size_t)argc - offset + 1, prog, __VA_ARGS__, NULL)
#define uv(...) exm(2, "uv", __VA_ARGS__)
#define ex1(offset, prog) manual_extend_exec(argv + offset, (size_t)argc - offset + 1, prog, NULL);

        if (argc == 1 || (argc == 2 && !strcmp(argv[1], "@"))) upanic("%s", USAGE);
        ...
        if (argc > 1 && !strcmp(argv[1], "s")) exm(2, get_venv("streamlit"), "run");
