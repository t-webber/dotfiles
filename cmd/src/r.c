...
static const char *single_work_file_runner(const_str file,
                                           const_str ext) {
        ...
        if (fd == NULL) panic("Failed to open %s.\n", file);
...
static int run_folder(const bool is_release) {
        ...
        if (buf == NULL)
                panic("Failed to get current working "
                      "directory\n");
