...
static void forked_exv(Args args) {
        pid_t pid = fork_checked();
        if (pid == 0)
                exv(args);
        fork_wait(pid);
...
static void copy_array(size_t *const idx, const char **const exec_args,
                       Args cli_args, const size_t cli_args_len) {
        for (size_t i = 0; i < cli_args_len; ++i)
                exec_args[(*idx)++] = cli_args[i];
}

static const char *single_work_file_runner(const char *const file,
        ...
        char *const line = malloc(sizeof(char *) * 64);
...
static void run_file(const char *const file, const char *const ext,
                     const char *const *const argv, const size_t argv_len,
        ...
        if (!strcmp(ext, "c") || !strcmp(ext, "cc") || !strcmp(ext, "cpp") ||
            !strcmp(ext, "rs")) {
                ...
                if (!strcmp(ext, "rs")) {
                        forked_exl("rustc", "rustc", file, "-o", "a.out",
                                   is_release ? "-C" : NULL, "opt-level=3");
                } else {
                        const char *const cmd =
                            strcmp(ext, "c") == 0 ? "cc" : "c++";
                        const char *const opt = is_release ? "-Ofast" : "-O0";
                        const char *const flag = is_release ? "-flto" : "-g";
        ...
        const char *const prog = single_work_file_runner(file, ext);
...
static const char *as_extension(const char *const file) {
        const char *ptr = file;
        for (; *ptr != '\0'; ++ptr)
                if (*ptr == '.')
                        return ptr + 1;
...
int main(const int argc, Args argv) {
        ...
        const char *const ext = as_extension(file);
