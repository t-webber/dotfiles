#pragma once

#include "lib.h"
#include <sys/types.h>
#include <unistd.h>

pid_t fork_checked(void);
int fork_wait(pid_t pid);
__wur int getdbg(void);

#define exl(first, ...)                                                        \
        {                                                                      \
                if (getdbg()) {                                                \
                        print_this_env();                                      \
                        print_inline_variadic(__VA_ARGS__);                    \
                        if (getdbg() == 2) upanic("exited for debug");         \
                }                                                              \
                epanic("Failed to execute %s: exited with code %d",            \
                       first,                                                  \
                       execlp(first, __VA_ARGS__))                             \
        }

#define exl1(cmd) exl(cmd, cmd, NULL);

#define exln(cmd, ...) exl(cmd, __VA_ARGS__, NULL);

#define exldn(cmd, ...) exl(cmd, cmd, __VA_ARGS__, NULL);

#define forked_exldn(cmd, ...)                                                 \
        if (fork_and_wait() == 0) exldn(cmd, __VA_ARGS__);

#define forked_exl1(cmd)                                                       \
        if (fork_and_wait() == 0) exl1(cmd);

#define pipestdout(fildes)                                                     \
        if (dup2(fildes, STDOUT_FILENO) < 0) {                                 \
                epanic("Failed to link stdout to writer");                     \
        }
#define pipestderr(fildes)                                                     \
        if (dup2(fildes, STDERR_FILENO) < 0) {                                 \
                epanic("Failed to link stderr to writer");                     \
        }
#define pipeboth(fildes)                                                       \
        pipestdout(fildes);                                                    \
        pipestderr(fildes);

#define __read_simple_exl_maker(__exl_func, buf_size, buffer, linker, ...)     \
        int fildes[2];                                                         \
        if (pipe(fildes)) { epanic("Failed to create fildes pipe"); }          \
                                                                               \
        pid_t pid = fork_checked();                                            \
                                                                               \
        if (pid == 0) {                                                        \
                if (close(fildes[0])) { epanic("Failed to free reader"); }     \
                linker(fildes[1]);                                             \
                if (close(fildes[1])) { epanic("Failed to free writer") };     \
                                                                               \
                __exl_func(__VA_ARGS__);                                       \
        }                                                                      \
                                                                               \
        if (close(fildes[1])) { epanic("Failed to free writer"); }             \
                                                                               \
        char *buffer = malloc(sizeof(char) * buf_size);                        \
                                                                               \
        ssize_t nbytes = 0;                                                    \
        ssize_t n;                                                             \
        while ((n = read(fildes[0],                                            \
                         buffer + nbytes,                                      \
                         buf_size - 1 - (size_t)nbytes))                       \
               > 0)                                                            \
                nbytes += n;                                                   \
                                                                               \
        buffer[nbytes] = '\0';                                                 \
                                                                               \
        fork_wait(pid);                                                        \
        if (close(fildes[0])) { epanic("Failed to free reader"); }             \
                                                                               \
        buffer[nbytes] = '\0';

#define read_simple_exldn(buf_size, buffer, ...)                               \
        __read_simple_exl_maker(exldn,                                         \
                                buf_size,                                      \
                                buffer,                                        \
                                pipestdout,                                    \
                                __VA_ARGS__)

#define read_simple_exldn_stderr(buf_size, buffer, ...)                        \
        __read_simple_exl_maker(exldn, buf_size, buffer, pipeboth, __VA_ARGS__)

#define read_simple_exl1(buf_size, buffer, ...)                                \
        __read_simple_exl_maker(exl1, buf_size, buffer, pipestdout, __VA_ARGS__)

#define read_simple_exvd(buf_size, buffer, cmd)                                \
        __read_simple_exl_maker(exvd, buf_size, buffer, pipestdout, cmd)

_Noreturn __nonnull() void exvd(Args args);
__nonnull() void forked_exvd(Args args);

_Noreturn __nonnull() void exl_notif(const_str message);

#define exl_corenotif(...)                                                     \
        {                                                                      \
                char message[128];                                             \
                sprintf(message, __VA_ARGS__);                                 \
                exl_notif(message);                                            \
        }

_Noreturn __nonnull() void exl_err_notif(const_str msg);

__wur pid_t fork_and_wait(void);
