...
#define usage                                                                                      \
        upanic(MAGENTA "Usage:" RED " %s <" YELLOW "file" RED ">(" YELLOW ".c" RED                 \
                       " [single|multi] || [<" YELLOW "folder" RED ">])",                          \
               argv[0])
...
#define normal                                                                                     \
        include(lib) "int main(const int argc, Args argv) {" tl                                    \
                     "store_usage(argv[0], \"\", false);" tl "return 0;" nl "}" nl

#define single include(libcmd) "make_single(\"g\", \"rg\", \"=.--hidden\", )" nl

#define multi                                                                                      \
        include(libcmd) "const Cmd C[] = {" tl "cmd(\"b\", \"build\", \"run\", )," nl "};" nl nl   \
                        "const Manual M[] = {" tl "{'?', \"--help\"}," tl                          \
                        "{'C', \"--color=always\"}," tl "{'-', \"--\"}," nl "};" nn                \
                        "make_settings(S, C, M);" nl "make_main(S, \"zig\", \"z\")" nl
