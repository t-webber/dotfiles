#include "lib.h" // contains the panic macro that prints and exits, and Args typedefed to const char*const*const
#include <errno.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>
...
static void
separate_icl_git_args(const int argc,
                      Args argv,
...
static void try_clone_url(const_str url,
                          const char **const git_args,
                          const bool exact) {
        printf("> Trying %s.\n", url);
...
#pragma GCC diagnostic ignored "-Wcast-qual"
        execv("/usr/bin/git", (char **const)git_args);
...
static void find_and_clone(const_str repo_name,
                           const char **const icl_args) {
        int pipefd[2];
        pipe(pipefd);
        pid_t pid = fork();
        if (pid == 0) {
                ...
                execl("/usr/bin/gh",
                      ...
                      repo_name,
                      NULL);
                panic("Failed to spawn subprocess for gh "
                      "s.\n");
        } else {
                int status;
                ...
                if (status)
                        panic("No repository named %s "
                              "found.\n",
                              repo_name);

                str url
                    = malloc(url_max_len * sizeof(char));
                const ssize_t len
                    = read(pipefd[0], url, url_max_len - 1);
                if (len <= 1)
                        panic("Unexpectedly found empty "
                              "remote url.\n");
...
/// name.
static void
try_clone_with_user(const char *user,
                    const char *repo,
                    const char **const git_args) {
...
static void parse_and_clone(const size_t icl_len,
        ...
        if (icl_len == 2) {
                try_clone_with_user(icl_args[0],
                                    icl_args[1],
                                    git_args);
                panic("Failed to find a git repository "
                      "with username %s and "
                      "name %s.\n",
                      icl_args[0],
                      icl_args[1]);
        ...
        if (!strncmp("http", icl_arg, 4)) {
                ...
                panic("%s isn't a valid git repository.\n",
                      icl_arg);
        }
        ...
        if (slash != NULL) {
                ...
                strncpy(user,
                        icl_arg,
                        (unsigned long)(slash - icl_arg));
                user[slash - icl_arg] = '\0';
                try_clone_with_user(user,
                                    slash + 1,
                                    git_args);
...
int main(const int argc, Args argv) {
        ...
        const char **const icl_args = malloc(
            ((unsigned long)argc + 1) * sizeof(char *));
        const char **const git_args = malloc(
            ((unsigned long)argc + 3) * sizeof(char *));
        size_t icl_len;

        separate_icl_git_args(argc,
                              argv,
                              &icl_len,
                              icl_args,
                              git_args);
