...
static int ls(const_str filename,
              const bool is_verbose,
              const bool has_multiple) {
        if (!is_verbose)
                return execlp("ls",
                              "ls",
                              "-A",
...
static int display_file(const_str filename,
                        const bool is_verbose) {
        const size_t len = strlen(filename);
        const_str extension
            = get_filename_extension(filename, len);
...
static int display(const_str filename,
                   const bool is_verbose,
                   const bool has_multiple) {
        struct stat st;
...
int main(const int argc, const_str *const argv) {
        ...
        if (argc == 1) {
                ...
                panic("Failed to read .: exec exited with "
                      "code %d: %s.\n",
                      res,
                      strerror(errno));
        }
        ...
        for (int file_nb = 1; file_nb < argc; ++file_nb) {
                ...
                if (pid < 0)
                        panic("Failed to fork the %d-th "
                              "time.\n",
                              file_nb);

                if (pid == 0) {
                        int res = display(argv[file_nb],
                                          verbose,
                                          argc > 2);
                        panic("Failed to read %s: exec "
                              "exited with code %d: "
                              "%s.\n",
                              argv[file_nb],
                              res,
                ...
                if (WIFEXITED(status)
                    && WEXITSTATUS(status) != 0)
                        panic("Child process failed with "
                              "status %d.\n",
                              WEXITSTATUS(status));
        }
}
