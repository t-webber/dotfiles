...
_Noreturn __nonnull((1)) static void macos_notif(const_str message, const char *title) {
        if (title == NULL) title = "C error";
        char osascript[512];
        sprintf(osascript, "display notification \"%s\" with title \"%s\"", message, title);
        exldn("osascript", "-e", osascript);
}

__nonnull() _Noreturn void exl_notif(const_str message) {
        if (is_file("/etc/artix-release")) exldn("herbe", "em:", message);
        macos_notif(message, NULL);
}

_Noreturn __nonnull() void exl_err_notif(const_str err_msg) {
        if (is_file("/etc/artix-release")) exldn("herbe", "br:#ff0000", err_msg);
