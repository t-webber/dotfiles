...
FILE *fopen_checked(const_str file_name, const_str mode) {
        ...
        if (f == NULL) panic("Failed to open %s", file_name);
        return f;
}

FILE *popen_checked(const_str command) {
        FILE *f = popen(command, "r");
        if (f == NULL) panic("Failed to run %s", command);
        return f;
}

const_str getenv_checked(const_str var) {
        const_str value = getenv(var);
        if (value == NULL) panic("Env var %s not defined.", var);
        return value;
}
...
void exl_err_notif(void) {
        exl("/bin/notify-send", "notify-send", "-u", "critical", "error");
}
