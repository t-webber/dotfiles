...
FILE *fopen_checked(const_str file_name, const_str mode) {
        ...
        if (f == NULL) epanic("Failed to open %s", file_name);
        return f;
}
...
const_str getenv_checked(const_str var) {
        ...
        if (value == NULL) epanic("Env var %s not defined.", var);
        return value;
}
...
void exl_err_notif(void) {
        exl_err_notif_msg("error");
}

void exl_err_notif_msg(const_str err_msg) {
        exl("/bin/notify-send", "notify-send", "-u", "critical", err_msg);
}
