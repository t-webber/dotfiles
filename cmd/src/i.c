...
#define cmd_num(name, alias)                                                                       \
        __attribute_malloc__ __wur static char *cmd_##name##_num(const int value) {                \
                if (strcmp(get_last_pushed(), #alias)) return cmd_head_num(value);                 \
...
#undef cmd
#define cmd(alias, expanded, ...) cmdfunc(alias, expanded, cmd_head_num, __VA_ARGS__)
    ...
    static const Cmd COMMANDS[] = {
        ...
        cmd("br",
            ...
            "--contains",
            "--show-current", ),
        cmd("bi", "bisect", "start", "bad", "good", "run", "=ereset", "visualize", "=a--stat", ),

        cmdfunc("cl", "clone", cmd_cl_num, "--depth", "--single-branch", "--branch", "--no-tags", ),

        cmd("ck", "checkout", "-t", "--orphan", "-b", "-B", ),
        ...
        cmd("c",
            ...
            "=w--date=now",
            "--date",
            "_=TGIT_COMMITTER_DATE", ),

        cmd("do", "difftool", "--cached", ),
        cmd("d", "diff", "--shortstat", "--cached", "--name-only", "=a--stat", ),
        ...
        cmd("fs", "fsck", "--no-reflogs", "--lost-found", ),
        ...
        cmd("lf", "lfs", "track", "install", "status", "ls-files", "--all", ),
        cmd("ls", "ls-files", "=r*.rs", ),
        cmdfunc("lr",
                "reflog",
                cmd_l_num,
                "--graph",
        ...
        cmd("n", "clean", "-f", "-f", "-d", "-x", "--recurse-submodules", "=y--dry-run", ),
        ...
        cmd("rb", "rebase", "--abort", "--continue", "--edit-todo", "--quit", "-i", "--root", ),
        ...
        cmd("y", "apply", "-R", "=s--ignore-space-change", "=w--ignore-whitespace", "--reject", ),
};
