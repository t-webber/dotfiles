...
static void exl_display(void) {
        ...
        exl_corenotif("%s%3d", emoji, level);
}
...
static void edit_volume(Args argv) {
        ...
        if (!up && strcmp(argv[1], "down"))
                exl_err_notif("Invalid command: expected up, down, start or kill");
...
int main(const int argc, Args argv) {
        ...
        if (argc == 2 && !strcmp(argv[1], "start")) {
                setenv_checked("DBUS_SESSION_BUS_ADDRESS", "unix:path=/dev/null");
                setenv_checked("DISABLE_RTKIT", "true");
                if (!fork()) exl1("pipewire");
                sleep(1);
