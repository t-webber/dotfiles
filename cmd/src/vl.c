...
int main(const int argc, Args argv) {
        ...
        if (argc == 2 && !strcmp(argv[1], "start")) {
                if (!fork_checked()) exldn("sudo", "/usr/lib/rtkit-daemon");
                if (!fork_checked()) exl1("pipewire");
                if (!fork_checked()) exl1("pipewire-pulse");
                if (!fork_checked()) exl1("wireplumber");
                return 0;
        }
        if (argc == 2 && !strcmp(argv[1], "kill")) {
                forked_exldn("sudo", "pkill", "rtkit-daemon");
