...
static char *get_path(void) {
        const_str config
            = getenv_checked("XDG_CONFIG_HOME");
...
static char *shorten_and_copy(char *restrict from,
                              char *restrict to,
                              char *restrict temp) {
...
static const char **
get_keybinds(size_t *const keybinds_index) {
        const char **const keybinds
        ...
        if (fd == NULL) panic("File not found: %s\n", path);
        free(path);
