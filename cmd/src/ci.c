...
static int install_git(const_str repo, const_str name) {
	printf("Installing repo %s with name %s...\n", repo, name);
	exl("cargo", "cargo", "install", "--git", repo, name, NULL);
}

static void usage(const_str arg0) {
	panic(
	    "Too many arguments.\nUsage: %s <[user] <crate>|<url>> [-n name]\n",
	    arg0);
}
...
static const_str get_repo(const_str url) {
...
int main(const int argc, Args argv) {
	...
	if (user != NULL) {
		...
		exl("cargo", "cargo", "binstall", "--git", repo, "-y");
	}
	...
	exl("cargo", "cargo", "binstall", "-y", crate);
}
