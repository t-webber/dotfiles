...
static size_t get_man_pages(bool only_c_pages, const_str page_name) {
...
static int mount(const int argc, Args argv) {
	if (argc > 4 || argc < 3)
		panic("Usage: %s [--mkdir] %s <destination>\n",
		      argv[0],
		      argv[1]);

	return execl("/usr/bin/sudo",
		     "sudo",
		     "mount",
		     argv[1],
		     argv[2],
...
int main(const int argc, Args argv) {
	...
	if (nb == 0)
		panic("man: No entry for %s in the manual.\n", argv[1]);
	return 0;
}
