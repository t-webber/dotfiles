...
static bool line_has_target(const_str line,
                            const_str target,
...
static int make(void) {
        exl("/usr/bin/make", "make");
}

static bool open_man_page(const_str full_path,
                          bool only_c_pages) {
...
static size_t
get_man_pages_in_folder(bool only_c_pages,
                        const_str page_name,
        ...
        DIR *d = opendir(path);
        if (d == NULL) panic("Failed to open %s.\n", path);
        ...
        while ((file = readdir(d))) {
                ...
                str full_path
                    = malloc(sizeof(char)
                             * (sizeof("/usr/share/man/")
                                + folder_name_len + 1
                                + file_name_len + 1));
                char *writer
                    = stpcpy(full_path, "/usr/share/man/");
...
static size_t get_man_pages(bool only_c_pages,
                            const_str page_name) {
...
static void parse_argv(int *const argc,
        ...
        *c_man = !strcmp(argv[0], "mc");
        ...
        if (!c_man && strcmp(argv[0], "m"))
                panic("Erroneous program name. Should be "
                      "either m or "
                      "mc, but "
                      "%s was found.\n",
                      argv[0]);
...
static int git_man_page(const int argc, Args argv) {
        assert(argc == 2);
        str page_name = malloc(30 * sizeof(char));
...
int main(const int argc, Args argv) {
        ...
        if (normalised_argc == 1) return make();
        ...
        if (nb == 0) exl("/usr/bin/make", "make", argv[1]);
        return 0;
}
