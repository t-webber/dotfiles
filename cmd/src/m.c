...
static bool line_has_target(const_str line, const_str target, const size_t target_len) {
...
static void try_make(const_str target, const_str makefile) {
        ...
        for (size_t i = 0; i < ALIASES_LEN; ++i) {
                ...
                if (!strcmp(alias[1], expanded_target)) upanic("use m %s", alias[0]);
        }
...
static size_t get_man_pages_in_folder(bool only_c_pages,
        ...
        while ((file = readdir(d))) {
                ...
                str full_path = (str)malloc(
                    sizeof(char)
                    * (sizeof("/usr/share/man/") + folder_name_len + 1 + file_name_len + 1));
...
static void parse_argv(int *const argc,
        ...
        *mount = *argc > 1 && argv[1][0] == '/';
        ...
        bool invalid_args = (*argc == 2 && !strcmp(argv[1], "?"))
                            || ((*argc > 4 || *argc < 3) && *mount) || (*argc > 3 && !*mount)
                            || (*c_man && *argc != 2);
        if (invalid_args) {
...
int main(const int argc, Args argv) {
        ...
        parse_argv(&normalised_argc, argv, &c_man, &git, &should_clear, &is_mount);
