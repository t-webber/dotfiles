...
static bool open_man_page(const_str full_path, bool only_c_pages) {
        ...
        if (display) { forked_exl("/usr/bin/man", "man", full_path); }
...
static size_t get_man_pages_in_folder(bool only_c_pages,
        ...
        DIR *d = opendir(path);
        if (d == NULL) panic("Failed to open %s", path);
        ...
        while ((file = readdir(d))) {
                ...
                str full_path
                    = malloc(sizeof(char)
                             * (sizeof("/usr/share/man/") + folder_name_len + 1
                                + file_name_len + 1));
...
static size_t get_man_pages(const bool only_c_pages, const_str page_name) {
        ...
        if (d == NULL) panic("Failed to open /usr/share/man");
...
static int mount(const int argc, Args argv) {
        ...
        exl("/usr/bin/sudo", "sudo", "mount", argv[1], argv[2], argv[3], NULL);
}
...
int main(const int argc, Args argv) {
        ...
        if (normalised_argc == 1) exl("/usr/bin/make", "make");
