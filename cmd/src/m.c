...
static void try_make(const char *const target, const char *const makefile) {
        ...
        while (fgets(line, 128, fd))
                if (line_has_target(line, expanded_target, target_len)) {
                        fclose(fd);
                        int res = execl("/usr/bin/make", "make", "-j20", "-f",
                                        makefile, expanded_target, NULL);
...
static void parse_argv(int *const argc, const char *const *const argv,
                       bool *const c_man, bool *const git, bool *const clear,
                       bool *const mount) {
...
static int mount(const int argc, Args argv) {
        ...
        return execl("/usr/bin/sudo", "sudo", "mount", argv[1], argv[2],
                     argv[3], NULL);
}

static int git_man_page(const int argc, Args argv) {
        assert(argc == 2);
        char *const page_name = malloc(30 * sizeof(char));
