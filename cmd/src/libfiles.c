...
__attribute_pure__ __wur __nonnull() const char *get_filename_extension(const_str filename) {
...
__wur __nonnull() __attribute_pure__ static bool is_valid_extension(const_str *const supported,
                                                                    const_str extension,
...
#define extensions(name, ...)                                                                      \
        ...
        __wur __nonnull() __attribute_pure__ static bool name##_supported(const_str extension) {   \
...
_Noreturn __nonnull() static void dezoom_and_run_alacritty(const_str filename, const bool is_mpv) {
...
#define mk(s, prefix)                                                                              \
        extend_const(&s, prefix);                                                                  \
        extend_s(&s, logs, logs_len);                                                              \
        push_s(&s, '/');                                                                           \
        extend_const(&s, "mpv." #s)

                mk(out, "--log-file=");
                mk(err, "2>");

                forked_exldn("mpv", "--vo=tct", "--msg-level=all=no", out.data, filename, err.data);
        } else {
...
_Noreturn __nonnull() void exec_open_file(const_str filename, const display_type ty) {
        ...
        if (terminal == TERMINAL_KITTY) {
                ...
                if (mpv_supported(extension)) exldn("mpv", "--vo=kitty", filename);
        }
        ...
        if (timg_supported(extension)) dezoom_and_run_alacritty(filename, false);
