...
const char *get_filename_extension(const_str filename,
                                   const size_t len) {
...
#define extensions(name, ...)                              \
        ...
        static bool name##_supported(                      \
            const_str extension) {                         \
                return is_valid_extension(name##_EXT,      \
                                          extension,       \
                                          name##_LEN);     \
        }
...
extensions(timg,
    ...
    extensions(mpv,
        ...
        extensions(sxiv,
            ...
            static void send_alacritty_config(
                const_str font_size) {
        pid_t pid = fork();
...
static int dezoom_and_run_alacritty(const_str filename,
                                    const bool is_mpv) {
        ...
        int stat;
        waitpid(pid, &stat, 0);
        if (!WIFEXITED(stat)) panic("Failed to execute.\n");
