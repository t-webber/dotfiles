...
static void new_sep(struct CharParsingState *const state, const char new) {
        if (state->sep != NO_SEP) {
                upanic("2 successive separators is invalid: %c followed by %c.", state->sep, new);
        }
...
__nonnull() static void print_cmd(const Cmd *const cmd,
        ...
        printf("%-*s %-*s ", (int)max_alias, cmd->alias, (int)max_expanded, cmd->expanded);
        for (const char **rdr = cmd->options; *rdr != NULL; ++rdr) { printf("[%s] ", *rdr); }
        printf("\n");
}

__wur __nonnull() static size_t get_max_alias(const Cmd *const cmds, const size_t cmd_len) {
        size_t max = 0;
        size_t current;
...
__wur __nonnull() static size_t get_max_expanded(const Cmd *const cmds, const size_t cmd_len) {
        size_t max = 0;
        size_t current;
        for (size_t i = 0; i < cmd_len; ++i) {
...
---\n\
");
        for (size_t i = 0; i < settings->manual_len; ++i) { print_manual(&settings->manual[i]); }
}

__wur __nonnull() static size_t
    find_command(const_str arg, size_t *start, const Cmd *const cmds, const size_t len) {
...
__nonnull() __wur
    static bool take_two(Vec *const cmd, Args opts, const char first, const char second) {
...
__nonnull() static void push_raw(struct CharParsingState *const state, Vec *const cmd) {
...
__nonnull() static void handle_char(Vec *const cmd,
        ...
        if (ch == '_') {
                if (state->israw) {
                        if (state->raw_mode.len == 0)
                                ++state->israw;
                        else if (state->closing_raw + 1 == state->israw)
                                push_raw(state, cmd);
                        else
                                ++state->closing_raw;
                } else
        ...
        if (state->closing_raw) {
                for (size_t ci = 0; ci < state->closing_raw; ++ci) push_s(&state->raw_mode, '_');
                state->closing_raw = 0;
        }
...
static void merge_sep(Vec *const cmd, const char sep) {
        ...
        if (pre_slash == NULL || cmd->len == 0)
                upanic("Tried to merge but no values aroud `%c`", sep);
        if (sep == ENV_SEP)
...
__nonnull() static void parse_alias(const CliSettings *const settings,
        ...
        if (has_command) {
                cmd_id = find_command(arg, &start, settings->cmd, settings->cmd_len);
        } else {
                cmd_id = 0;
        }
...
__nonnull() void print_vec(Vec *vec) {
        for (size_t idx = 0; idx < vec->len; ++idx) { printf("%s ", vec->data[idx]); }
        printf("\n");
}
